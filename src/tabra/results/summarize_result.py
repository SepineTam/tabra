...
class SummarizeResult(BaseResult):
    ...
    def _summary_detail_stata(self):
        ...
        for name in self._var_names:
            ...
            lines.append(f"{'1%':>12.4g} {p.get('1%', 0):>12.4g}")
            lines.append(f"{'5%':>12.4g} {p.get('5%', 0):>12.4g}")
            lines.append(f"{'10%':>12.4g} {p.get('10%', 0):>12.4g} {'Obs':>20s} {self._obs[name]:>10d}")
            lines.append(f"{'25%':>12.4g} {p.get('25%', 0):>12.4g} {'Sum of Wgt.':>20s} {self._obs[name]:>10d}")
            lines.append("")
            lines.append(f"{'50%':>12.4g} {'Mean':>20s} {self._mean[name]:>12.4g}")
            lines.append(f"{'Largest':>12s} {'Std. dev':>20s} {self._std[name]:>12.4g}")
            lines.append(f"{'75%':>12.4g} {p.get('75%', 0):>12.4g}")
            lines.append(f"{'90%':>12.4g} {p.get('90%', 0):>12.4g} {'Variance':>20s} {self._std[name]**2:>12.4g}")
            lines.append(f"{'95%':>12.4g} {p.get('95%', 0):>12.4g} {'Skewness':>20s} {self._skewness.get(name, 0):>12.4g}")
            lines.append(f"{'99%':>12.4g} {p.get('99%', 0):>12.4g} {'Kurtosis':>20s} {self._kurtosis.get(name, 0):>12.4g}")
            lines.append("")
